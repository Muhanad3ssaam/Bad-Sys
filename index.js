    ...
    const embed = new Discord.RichEmbed()
      ...
      .setDescription(`
         ...
         - ${prefix}embed : كلام بامبد جميل
          ...
          - ${prefix}invite : لأضافه البوت لسيرفرك
         \`Voice Commands : \`
        ...
        - ${prefix}credit : لمعرفه رصيدك

        - ${prefix}daily : لسحب جائزه يوميه

         لتحويل رصيد الى شخص
        - ${prefix}credit <المبلغ> <منشن للشخص> 
         \`\`\`music Commands  :\`\`\`  
          - ${prefix}play :لتشغيل اغنيه
          - ${prefix}pause : لتوقيف الاغنيه مؤقتا
          - ${prefix}stop : للايقاف
          - ${prefix}skip : لتخطي الاغنيه
          - ${prefix}np : لاعرض الأغنية التي تعمل في وقتك الحالي
          - ${prefix}playlist : لتشغيل قوائم الموسيقي في اليوتيوب عن طريق البحث او الرابط
          - ${prefix}lyrics : احصل على كلمات الأغنية التي يتم تشغيلها حاليًا (ان وجدت)
          \`\`\`other Commands  :\`\`\`  
         
         - ${prefix}تقديم : للتقديم
         - ${prefix}sug : لوضع اقتراح
         - ${prefix}news : لعرض أحدث اخبار البوت
         - ${prefix}addemoji **emoji name** **emoji link**
      ...
      message.channel.send(`**:heavy_dollar_sign:| \`${number}\`, أكتب الرقم للأستمرار**`).then(m => {
        message.channel.awaitMessages(m => m.author.id === message.author.id, { max: 1, time: 10000 }).then(c => {
          if (c.first().content === number) {
            ...
            credits[mention.id].credits += (+balance); console.log(`Collected ${massage.content}`);
            fs.writeFile(path, JSON.stringify(credits, null, 5), function(err) { if (err) console.log(err) });
